...
bootstrap_analysis <- function(data, formula_str, Number_of_boots = 1000, output_file = "bootstrapped_effect_sizes.jpg") {
  ...
  for(i in 1:Number_of_boots) {
    ...
    BS_lm <<- lm(formula = formula_str, data = BS_X)
    
    BS_params[i,] <- coef(BS_lm) %>%
      t() %>%
      as.data.frame()
  }
  ...
  ci_plot <- plot_model(m1, type = "std") + ylim(-1.5, 1) + theme_cowplot() + labs(title = ci_title) + theme(plot.title = element_text(size = 18))
...
# Usage
# bootstrap_analysis(data = d, formula_str = "y ~ x*z", output_file = "my_custom_name.jpg")